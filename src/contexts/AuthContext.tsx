...
async function fetchAppUser(supabaseUser: SupabaseUser): Promise<AppUser | null> {
  // FORCE L'ADMINISTRATION POUR TON EMAIL
  if (supabaseUser.email === "elmaghriothmane@elcdeparis.fr") {
    return {
      id: supabaseUser.id,
      email: supabaseUser.email,
      fullName: "Othmane (Directeur)",
      role: "admin_gestionnaire",
      approved: true,
      soldeConges: { total: 25, utilise: 0, restant: 25 },
    };
  }

  // Récupération normale pour les autres utilisateurs
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signup = async (email: string, password: string, fullName: string): Promise<string | null> => {
    // Restriction aux emails @elcdeparis.fr
